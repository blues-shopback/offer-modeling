...
class CustomWeightSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(self, start_value, min_value, min_steps):
        super().__init__()

        self.start_value = start_value
        self.min_value = min_value
        self.min_steps = float(min_steps)

    def __call__(self, step):
        step = tf.cast(step, tf.float32)
        weight = tf.cond(
            step >= self.min_steps,
            lambda: self.min_value,
            lambda: self.start_value * (1 - step / self.min_steps)
        )
        weight = tf.math.maximum(weight, self.min_value)

        return weight


# Ref: https://github.com/tensorflow/models/blob/master/official/nlp/optimization.py
class AdamWeightDecay(tf.keras.optimizers.Adam):
    ...
    def __init__(self,
        ...
        if logger:
            logger.info('gradient_clip_norm=%f', gradient_clip_norm)

    def _prepare_local(self, var_device, var_dtype, apply_state):
        super(AdamWeightDecay, self)._prepare_local(
            var_device, var_dtype,  # pytype: disable=attribute-error  # typed-keras
            apply_state)
