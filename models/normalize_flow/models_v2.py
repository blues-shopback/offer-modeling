...
class FlowConfig(config_utils.BaseConfig):
    @staticmethod
    def get_keys():
        return ["n_layer", "d_model", "d_embed", "n_head", "d_head", "d_inner",
                "dropout", "dropatt", "pre_ln",
...
class LocScaleLayer(tf.Module):
    ...
    def __call__(self, inp):
        ...
        if not hasattr(self, "attn_layer"):
            self.attn_layer = realnvp.AttnLayer(config, self.dtype)
            self.loc_dense = tf.keras.layers.Dense(
                    config.d_model, activation=None, dtype=self.dtype,
                    kernel_initializer=self.initializer, name="loc_dense")

            self.scale_dense = tf.keras.layers.Dense(
                    config.d_model, activation=tf.math.softplus, dtype=self.dtype,
                    kernel_initializer=self.initializer, name="scale_dense")

        fw_out, bw_out = self.attn_layer(inp, bi_causal_mask=True)

        fw_out = tf.transpose(fw_out, [1, 0, 2])
        bw_out = tf.transpose(bw_out, [1, 0, 2])

        fw_loc = self.loc_dense(fw_out)
        fw_scale = self.scale_dense(fw_out)
        bw_loc = self.loc_dense(bw_out)
        bw_scale = self.scale_dense(bw_out)

        return fw_loc, fw_scale, bw_loc, bw_scale


class FinModel(tf.Module):
    def __init__(self, config, flow_config, dtype, name="fin_model", is_training=True):
        super().__init__(name=name)
        self.config = config
    ...
    def __call__(self, inp, ids, inp_mask=None):
        ...
        if not hasattr(self, "realnvp_layers"):
            self.realnvp_layers = FinRealNVPModel(
                flow_config, self.dtype, name="fin_realnvp", is_training=self.is_training)
        # shape: [sample, bsz, seq, d_model]
        flow1_out, loglikeihood = self.realnvp_layers(inp, sample_size=flow_config.sample_size)
...
class FinRealNVPModel(tf.Module):
    ...
    def __call__(self, inp, inp_mask=None, sample_size=1):
        ...
        if not hasattr(self, "loc_scale_layer"):
            self.loc_scale_layer = LocScaleLayer(
                config, self.dtype, is_training=self.is_training
            )
        # reshape to [bsz, qlen, value_num*value_d]
        inp_reshape = tf.reshape(inp, shape=[bsz, qlen, -1])

        fw_loc, fw_scale, bw_loc, bw_scale = self.loc_scale_layer(inp_reshape)
        ...
        loc_next = fw_loc[:, 1:, :]
        scale_next = fw_scale[:, 1:, :]
        loc_last = bw_loc[:, :-1, :]
        scale_last = bw_scale[:, :-1, :]
        ...
        flow1_out, log_s1 = self.flow1(
            fw_loc, fw_scale, direction="forward", sample_size=sample_size)
        flow2_out, log_s2 = self.flow2(
            bw_loc, bw_scale, direction="forward", sample_size=sample_size)
