        ...
        if seg_id is not None:
            if untie_r:
                ...
                r_s_bias = tf.get_variable('r_s_bias', [n_head, d_head],
                                           dtype=tf_float, initializer=initializer)
            # modify 2 -> 3
            seg_embed = tf.get_variable('seg_embed', [n_layer, 3, n_head, d_head],
                                        dtype=tf_float, initializer=initializer)
            ...
            seg_mat = tf.cast(
                    ...
                    tf.int32)
            seg_mat = tf.one_hot(seg_mat, 3, dtype=tf_float)
        else:
            seg_mat = None
...
def lm_loss(hidden, target, n_token, d_model, initializer, lookup_table=None,
            tie_weight=False, bi_data=True, use_tpu=False, scope="lm_loss"):
    """doc."""
